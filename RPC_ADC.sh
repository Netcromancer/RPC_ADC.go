...
install_tools() {
    ...
    TARGETPKGS="debootstrap ifenslave ifenslave-2.6 lsof lvm2 ntp ntpdate openssh-server sudo tcpdump vlan"
    apt-get -qq update

    if [[ ${VERSION} == 14.04 && ${RS_SERVER_NAME} =~ 'infra01' ]]; then
    ...
    if [[ $CHECK_RECAP -eq 0 ]]; then
        ...
        (cd ${ADC_DIR}/recap/ && make install &>/dev/null && sleep 5 \
            && sed -i "s@^LOG_EXPIRY=.*@LOG_EXPIRY=30@" /etc/recap \
            && sed -i "s@^EMAIL_ON_ERROR=.*@EMAIL_ON_ERROR=0@g" /etc/recap \
            && for i in USESAR USESARR USESARQ USENETSTAT USENETSTATSUM; \
                do sed -i "s@^${i}=.*@${i}=yes@" /etc/recap; done \
    ...
    (cp /etc/default/grub.d/kexec-tools.cfg ${BACKUP_DIR}/kexec-tools.cfg.bak \
            && sed -i 's@384@1024@g' /etc/default/grub.d/kexec-tools.cfg \
...
install_openmanage() {
    ...
    if [[ -f ${SOURCES_FILE} ]] && 
        ...
        echo "deb http://linux.dell.com/repo/community/ubuntu ${CODENAME} openmanage/830" \
...
install_hp_mcp() {
    ...
    SOURCES_FILE="/etc/apt/sources.list.d/linux.hp.com.sources.list"
    CODENAME=$(lsb_release -c | awk '{print $NF}')

    if [[ -f ${SOURCES_FILE} ]] && 
        [[ $(grep "linux.hpe.com" ${SOURCES_FILE}) ]]; then
        echo -e "${INFORM}\tHP MCP repository already installed"
    else
        if [[ ${CODENAME} == trusty ]]; then
            echo "deb http://downloads.linux.hpe.com/SDR/repo/mcp/ubuntu ${CODENAME}/current non-free" \
    ...
    for FINGERPRINT in 5CE2D476 B1275EA3
    do   
    ...
    # Install packages
    MCPPKGS="hp-health hponcfg hpssacli hp-snmp-agents"
    apt-get -qq update
...
update_hp_bios() {
    # Ensure that the HP performance profile is set
...
verify_crashdump_memory_alloc() {
    (grep -o 1024M /proc/cmdline &> /dev/null \
            && grep -o 1024M /etc/default/grub.d/kexec-tools.cfg  &> /dev/null \
...
verify_bios() {
    if [[ ${VENDOR} == Dell ]]; then
        ...
        if [[ ${cfgSerialConsoleEnable} == 1 ]]; then
